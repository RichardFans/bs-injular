...
(function newInstance(window, document) {
  ...
  function cleanScope(scope, templateElements, template, $compile, angular) {
    ...
    function watcherInterceptor() {
      ...
      if (scope.$$watchers && scope.$$watchers.length > 0) addedWatchers.push(scope.$$watchers[0]); // fan
      deregister();
      return deregister;
    }
    ...
    function listenerInterceptor(name, listener) {
      ...
      deregister();
      if (scope.$$listeners[name]) scope.$$listeners[name].pop(); // fan
      return deregister;
    }
  }
  ...
  function isListenerFromTemplate(listener) {
    ...
    var listenerString = listener ? listener.toString() : ''; // fan
